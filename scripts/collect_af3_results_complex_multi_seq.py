        ...
        for model_0_dir in sorted(af3_output_root.glob(args.wildcard + "_id_0_apo")):
            ...
            while os.path.isdir(model_x_dir):
                ...
                if all_states_completed:
                    print(f"Processing {model_x_dir.name} and corresponding holo and complex models...")
                    ...
                    if apo_summary_confidences["pass"] and \
                            ...
                            holo_summary_confidences["ipae_min"] < args.ipae_min_cut and \
                            ...
                            complex_metrics["mean_plddt"] > 85 and \
                        ...
                        print(f"Model {model_x_dir.name} passed all filters for apo, holo, and complex states.")
                        try_insert_best_ipae(
                            holo_metrics["mean_ipae"],
                            holo_model_x_dir,
                            best_ipae_list,
                            best_ipae_holo_model_dirs,
                        )
                        print(f"Updated best iPAE list: {best_ipae_list}")
                        print(f"Updated best iPAE holo model dirs: {best_ipae_holo_model_dirs}")
                # elif apo_conf:
                #     if apo_summary_confidences["pass"] and \
                #             apo_frac_gt90 > args.plddt_frac_gt90_cut and \
            ...
            for ipae, model_dir in zip(best_ipae_list, best_ipae_holo_model_dirs):
                if model_dir is None:
                    ...
                    print(f"Selected model with iPAE {ipae:.2f} from {model_dir}")
                ...
                holo_cif = next(model_dir.glob("*.cif"))
                ...
                best_complex_model_dir = model_dir.with_name(model_dir.name[:model_dir.name.rfind("holo")] + "complex")
                complex_cif = next(best_complex_model_dir.glob("*.cif"))
                ca_mob, mob_struct = load_ca_atoms(complex_cif)
                # if args.no_copy_pdb or not args.no_copy_pdb and not complex_cif.name in folded_designs:
                #     print(complex_cif)
                #     print(best_complex_plddt)
                if not args.no_copy_pdb and not complex_cif.name in folded_designs:
                    # Align
                    sup = Superimposer()
                    sup.set_atoms(ca_ref[:len(ca_mob)], ca_mob)
                    sup.apply(mob_struct.get_atoms())
