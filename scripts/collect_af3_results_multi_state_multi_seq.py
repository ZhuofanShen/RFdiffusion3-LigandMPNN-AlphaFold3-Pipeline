...
def try_insert_best_ipae(
    current_ipae: float,
    current_model_dir: Path,
    best_ipae_list: list,
    best_ipae_holo_model_dirs: list,
):
    """
    Maintain best (lowest) iPAE values and corresponding model dirs.
    Lists are modified in place.
    """
    n = len(best_ipae_list)

    # If not better than the worst cached value, skip
    if current_ipae >= max(best_ipae_list):
        return

    # Find insertion index
    insert_idx = None
    for i, ipae in enumerate(best_ipae_list):
        if current_ipae < ipae:
            insert_idx = i
            break
    ...
    # Insert and trim
    best_ipae_list.insert(insert_idx, current_ipae)
    best_ipae_holo_model_dirs.insert(insert_idx, current_model_dir)

    del best_ipae_list[n:]
    del best_ipae_holo_model_dirs[n:]
...
def main():
    ...
    ap.add_argument("--mean_plddt_cut", type=float, default=82.0)#88.0
    ap.add_argument("--ligand_mean_plddt_cut", type=float, default=60.0)#60.0 84.5
    ap.add_argument("--plddt_frac_gt90_cut", type=float, default=0.76)#0.76 0.82
    ap.add_argument("--mean_ipae_cut", type=float, default=6.0)#3.0
    ap.add_argument("--ptm_cut", type=float, default=0.79)#0.86
    ap.add_argument("--iptm_cut", type=float, default=0.71)#0.71 0.88
    ap.add_argument("--ipae_min_cut", type=float, default=2.50)#1.30
    ap.add_argument("--max_disorder", type=float, default=0.1)
    ap.add_argument("-n", "--n_sequences", type=int, default=10)
        ...
        for model_0_dir in sorted(af3_output_root.glob(args.wildcard + "_id_0_apo")):
            ...
            while os.path.isdir(model_x_dir):
                ...
                if all_states_completed:
                    ...
                    # print(min(holo_frac_gt90_list))
                    # if min(holo_frac_gt90_list) > args.plddt_frac_gt90_cut:
                    #     print(True)
                    # else:
                    #     print(False)
                    ...
                    # print(pass_holo_summary_confidences)
                    ...
                    if apo_summary_confidences["pass"] and \
                            ...
                            max(ipae_min_list) < args.ipae_min_cut:

                        # apo_cif = next(model_x_dir.glob("*.cif"))
                        ...
                        #     print(apo_cif)
                        #     print(apo_metrics["mean_plddt"])
                        # if not args.no_copy_pdb and not apo_cif.name in folded_designs:
                        #     # Align
                        #     sup = Superimposer()
                        #     sup.set_atoms(ca_ref[:len(ca_mob)], ca_mob[:len(ca_ref)])
                        ...
                        # for ligand_cif in args.ligand_cifs:
                        #     holo_model_x_dir = model_x_dir.with_name(model_x_dir.name[:-3] + "holo_" + ligand_cif.stem)
                        #     holo_cif = next(holo_model_x_dir.glob("*.cif"))
                        #     ca_mob, mob_struct = load_ca_atoms(holo_cif)
                        #     if args.no_copy_pdb or not args.no_copy_pdb and not holo_cif.name in folded_designs:
                        #         print(holo_cif)
                        #         holo_conf = next(filter(lambda x: not x.name.endswith("_summary_confidences.json"), \
                        #             holo_model_x_dir.glob("*_confidences.json")), None) if holo_model_x_dir.exists() else None
                        #         holo_conf_json = load_af3_conf(holo_conf)
                        #         holo_metrics = load_mean_plddt_ipae_confidence(holo_conf_json, ligand=True)
                        #         print(holo_metrics["mean_plddt"])
                        #     if not args.no_copy_pdb and not holo_cif.name in folded_designs:
                        #         # Align
                        #         sup = Superimposer()
                        #         sup.set_atoms(ca_ref[:len(ca_mob)], ca_mob[:len(ca_ref)])
                        #         sup.apply(mob_struct.get_atoms())
                        ...
                        current_ipae = mean_ipae_list[0]
                        try_insert_best_ipae(
                            current_ipae,
                            holo_model_x_dir,
                            best_ipae_list,
                            best_ipae_holo_model_dirs,
                        )
            ...
            for ipae, model_dir in zip(best_ipae_list, best_ipae_holo_model_dirs):
                if model_dir is None:
                    continue

                args.analysis_output_root.mkdir(exist_ok=True)
                with open(args.analysis_output_root / (args.ligand_cifs[0].stem + ".log"), "a") as pf:
                ...
                for ligand_cif in args.ligand_cifs:
                    best_holo_model_dir = model_dir.with_name(model_dir.name[:model_dir.name.rfind("holo")] + "holo_" + ligand_cif.stem)
    ...
    # selectivity
    r = str(round(np.corrcoef(list(holo_mean_plddts[0]), list(holo_mean_plddts[1]))[0, 1], 2))
    plot_scatter(list(holo_mean_plddts[0]), list(holo_mean_plddts[1]), "R average pLDDT", 
                 "S average pLDDT", "Holo R vs S pLDDT correlation", 
                 args.analysis_output_root / str("holo_R_vs_S_plddt_scatter_" + r + ".png"))
    r = str(round(np.corrcoef(list(holo_ptms[0]), list(holo_ptms[1]))[0, 1], 2))
    plot_scatter(list(holo_ptms[0]), list(holo_ptms[1]), "R average pTM", 
                 "S average pTM", "Holo R vs S pTM correlation", 
                 args.analysis_output_root / str("holo_R_vs_S_ptm_scatter_" + r + ".png"))
    r = str(round(np.corrcoef(list(ligand_mean_plddts[0]), list(ligand_mean_plddts[1]))[0, 1], 2))
    plot_scatter(list(ligand_mean_plddts[0]), list(ligand_mean_plddts[1]), "R ligand pLDDT", 
                 "S ligand pLDDT", "Holo R vs S ligand pLDDT correlation", 
                 args.analysis_output_root / str("holo_R_vs_S_ligand_plddt_scatter_" + r + ".png"))
    r = str(round(np.corrcoef(list(mean_ipaes[0]), list(mean_ipaes[1]))[0, 1], 2))
    plot_scatter(list(mean_ipaes[0]), list(mean_ipaes[1]), "R average iPAE", 
                 "S average iPAE", "Holo R vs S average iPAE correlation", 
                 args.analysis_output_root / str("holo_R_vs_S_mean_ipae_scatter_" + r + ".png"))
    r = str(round(np.corrcoef(list(ipae_min_vals[0]), list(ipae_min_vals[1]))[0, 1], 2))
    plot_scatter(list(ipae_min_vals[0]), list(ipae_min_vals[1]), "R iPAE min", 
                 "S iPAE min", "Holo R vs S iPAE min correlation", 
                 args.analysis_output_root / str("holo_R_vs_S_ipae_min_scatter_" + r + ".png"))
    r = str(round(np.corrcoef(list(iptms[0]), list(iptms[1]))[0, 1], 2))
    plot_scatter(list(iptms[0]), list(iptms[1]), "R ipTM", 
                 "S ipTM", "Holo R vs S ipTM correlation", 
                 args.analysis_output_root / str("holo_R_vs_S_iptm_scatter_" + r + ".png"))
